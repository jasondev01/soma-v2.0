...
export default function SelectEpisode({ info, episodes, currentEpisode }: Props) {
    ...
    return (
        <div className="flex gap-2">
            <div className="w-fit h-full text-xs tracking-wide font-medium md:font-semibold !select-none relative">
                ...
                {clicked && (
                    <div className={`uppercase gap-x-1 bg-slate-800 mt-1 md:mt-2 origin-bottom relative rounded-md !z-[999] flex flex-col  ${ episodes.length > 12 ? "overflow-y-auto h-60" : "h-fit" }`}
                    >
                        {episodes
                            .slice()
                            .reverse()
