...
export default function CharacterCard({ data: char }: Props) {
    ...
    return (
        <div className="h-[250px] block relative group overflow-hidden border border-transparent hover:border-cyan-300 hover:shadow-[0px_0px_5px_1px] hover:shadow-cyan-300 transition-all duration-300 rounded-md"
        ...
        >
            ...
            />
            <img src={char?.voiceActors[0]?.image}
                alt={`${char?.voiceActors[0]?.name?.full || '/no_image.png'}`}
                width={250}
                height={250}
            ...
            />
            <div className="absolute z-[2] bottom-0 left-0 w-full p-2 bg-gradient-to-b from-transparent to-black flex flex-col gap-y-[1px]">
                <span className="text-sm font-semibold block group-hover:hidden">
                    {char?.name?.full}    
                </span>
